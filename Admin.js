...
import { getFirestore, doc, getDoc, setDoc, deleteDoc, collection, getDocs } from "https://www.gstatic.com/firebasejs/10.11.0/firebase-firestore.js";
...
function addCardToPool(card, rarity = "Common") {
  ...
  cardEl.classList.add('card-thumbnail');
...
function renderCardGrid(cards = cardData) {
  ...
  pageCards.forEach(card => {
    ...
    cardImg.onload = () => {
      cardImg.classList.add('visible'); // Trigger fade-in after load
    };
...
selectUserBtn.addEventListener("click", loadUserList);
